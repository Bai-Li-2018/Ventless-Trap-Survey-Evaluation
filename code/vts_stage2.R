#######################################
## Ventless Trap Survey Project
## Evaluating the effectiveness of current Ventless Trap Survey in monitoring lobster stock in a changing Gulf of Maine
## Option I:Project lobster distribution using the Inshore Bottom Trawl Survey data and use the current Ventless Trap survey design to sample the simulated "true" population. Identify the optimal sampling design and sample size by mimicking how the data are collected in the Ventless Trap Survey.
## Step 1: Simulate lobster population using GAM
## Step 2: Project future lobster population using GAM
## Step 3: Evaluate the effectiveness of the current Ventless Trap Survey design in monitoring lobster stock through simulation approach
## Option II: Use the collected data as "true" population and identify the optimal sampling design and sample size by simulating various sampling strategies.
...
depth_grid_plot <- read.csv("./output/grid_depth_data.csv")
prediction_points <- SpatialPoints(cbind(depth_grid_plot$lon[which(depth_grid_plot$fathom<0)], depth_grid_plot$lat[which(depth_grid_plot$fathom<0)]), proj4string=CRS("+proj=longlat +datum=WGS84")) 
vts_points <- SpatialPoints(cbind(validation_vts_vtrap_data$LONGITUDE_DECIMAL, validation_vts_vtrap_data$LATITUDE_DECIMAL), proj4string=CRS("+proj=longlat +datum=WGS84"))
